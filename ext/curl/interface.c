...
static zval *curl_write_property(zend_object *object, zend_string *name, zval *value, void **cache_slot)
{
	...
	if (zend_string_equals_literal(name, "share")) {
		zval *property = zend_std_read_property(object, name, BP_VAR_RW, NULL, NULL);
		ZVAL_COPY(&old_value, property);
	}
	...
	if (zend_string_equals_literal(name, "share")) {
		...
		case IS_NULL:
			if (curl_easy_setopt(ch->cp, CURLOPT_SHARE, NULL) != CURLE_OK) {
				zend_std_write_property(object, name, &old_value, cache_slot);
				zend_throw_error(NULL, "Failed to unset share handle");
				return &EG(error_zval);
			}
			break;
		case IS_OBJECT:
			ZEND_ASSERT(Z_OBJCE_P(result) == curl_share_ce || Z_OBJCE_P(result) == curl_share_persistent_ce);
			php_curlsh *sh = Z_CURL_SHARE_P(result);
			if (curl_easy_setopt(ch->cp, CURLOPT_SHARE, sh->share) != CURLE_OK) {
				zend_std_write_property(object, name, &old_value, cache_slot);
